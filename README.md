...
**deploy-terraform** writes a temp `ci.auto.tfvars.json` from `variables-json` and applies with `-var-file`. `do-token` is merged into the JSON as `do_token` automatically. S3 backend credentials are passed via `terraform init -backend-config` (`access_key`, `secret_key`, `key`, `region`) — not as Terraform variables. App Terraform modules must declare `backend "s3"` with bucket and partial config; the action overrides `key` and `region` at init.
